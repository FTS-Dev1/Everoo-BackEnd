...
// Models :
const VeranstModel = require("../../model/veranstaltungstechnik");
...
const CreateData = catchAsync(async (req, res, next) => {
    try {
        ...
        const newData = new VeranstModel(payload);
        await newData.save();
...
const UpdateData = catchAsync(async (req, res, next) => {
    try {
        ...
        let result = await VeranstModel.findByIdAndUpdate(id, payload, { new: true })
...
const getData = catchAsync(async (req, res) => {
    try {
        const result = await VeranstModel.find();
...
const deleteData = catchAsync(async (req, res) => {
    try {
        let { id } = req.params;
        const result = await VeranstModel.findByIdAndDelete(id);
