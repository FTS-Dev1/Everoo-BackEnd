...
const CreateData = catchAsync(async (req, res, next) => {
    try {
        ...
        const newData = new DekorationModel(payload);
        await newData.save();
...
const UpdateData = catchAsync(async (req, res, next) => {
    try {
        ...
        let result = await DekorationModel.findByIdAndUpdate(id, payload, { new: true })
...
const getData = catchAsync(async (req, res) => {
    try {
        const result = await DekorationModel.find();
...
const deleteData = catchAsync(async (req, res) => {
    try {
        let { id } = req.params;
        const result = await DekorationModel.findByIdAndDelete(id);
