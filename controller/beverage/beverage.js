...
const CreateData = catchAsync(async (req, res, next) => {
    try {
        ...
        const newData = new BeverageModel(payload);
        await newData.save();
...
const UpdateData = catchAsync(async (req, res, next) => {
    try {
        ...
        let result = await BeverageModel.findByIdAndUpdate(id, payload, { new: true })
...
const getData = catchAsync(async (req, res) => {
    try {
        const result = await BeverageModel.find();
...
const deleteData = catchAsync(async (req, res) => {
    try {
        let { id } = req.params;
        const result = await BeverageModel.findByIdAndDelete(id);
