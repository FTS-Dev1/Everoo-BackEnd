...
const CreateData = catchAsync(async (req, res, next) => {
    try {
        ...
        const newData = new CateringModel(payload);
        await newData.save();
...
const UpdateData = catchAsync(async (req, res, next) => {
    try {
        ...
        let result = await CateringModel.findByIdAndUpdate(id, payload, { new: true })
...
const getData = catchAsync(async (req, res) => {
    try {
        const result = await CateringModel.find();
...
const deleteData = catchAsync(async (req, res) => {
    try {
        let { id } = req.params;
        const result = await CateringModel.findByIdAndDelete(id);
